...
func ConvertImage(imageData *bytes.Buffer, fileName string, doOutput bool, scaleFactor float64, width int, height int) (*bytes.Buffer, error) {
	...
	mainSheetName = path.Base(fileName)
	...
	for y := 0; y < imageHeight; y++ {
		...
		f.SetConditionalFormat(mainSheetName, fmt.Sprintf("%s:%s", rowR, rowEndR), []excelize.ConditionalFormatOptions{conditionalFormatRed})

		s.SetRow(rowG, append(getImageRow(resized, y, Green), 0, 255))
		f.SetConditionalFormat(mainSheetName, fmt.Sprintf("%s:%s", rowG, rowEndG), []excelize.ConditionalFormatOptions{conditionalFormatGreen})

		s.SetRow(rowB, append(getImageRow(resized, y, Blue), 0, 255))
		f.SetConditionalFormat(mainSheetName, fmt.Sprintf("%s:%s", rowB, rowEndB), []excelize.ConditionalFormatOptions{conditionalFormatBlue})
